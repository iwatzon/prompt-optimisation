...
class ChatOllamaModel(LMMBaseModel):
    """
    ChatOllama specific implementation of the LMMBaseModel.

    This class provides a common interface for the ChatOllama language model and includes methods for prediction.

    Parameters:
    -----------
    model : str
        The name of the ChatOllama language model.
    max_new_tokens : int
        The maximum number of new tokens to be generated.
    temperature : float
        The temperature for text generation (default is 0).
    ...
    def __init__(self, model_name, max_new_tokens, temperature, device, dtype):
        super(ChatOllamaModel, self).__init__(model_name, max_new_tokens, temperature)
        self.model_name = model_name
        self.temperature = temperature
    ...
    async def __call__(self, input_text, **kwargs):
        return await self.predict(input_text, **kwargs)
